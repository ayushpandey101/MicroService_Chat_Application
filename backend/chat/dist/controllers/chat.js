...
export const sendMessage = TryCatch(async (req, res) => {
    ...
    if (imageFile) {
        const uploadedImageUrl = imageFile.path || imageFile.secure_url || imageFile.url;
        const uploadedImagePublicId = imageFile.filename || imageFile.public_id || "";
        if (!uploadedImageUrl) {
            res.status(500).json({
...
export const getMessagesByChat = TryCatch(async (req, res) => {
    ...
    const isUserInChat = chat.users.some((u) => u.toString() === userId.toString());
    if (!isUserInChat) {
        res.status(403).json({
            message: "You are not a participants of this chat",
        });
